    ...
    def get_movies(self, sc):
        ...
        for movProg in moviePrograms:
            ...
            for m in movies:    #have to take movie property duration, to compare with moviePrograms
                if false:   #IMPORTANT: CHECK CONNECTION! BETWEEN MOVIE AND MOVIEPROGRAM    movProg.name == m
                    if.movProg.scheduleMovieTime[0]+movie.duration[0] < finishBefore:
        ...
        for opti in options2:
            for opt in options:
                if opti == opt:     #IMPORTANT: CHECK CONNECTION! BETWEEN MOVIE AND MOVIEPROGRAM
